...
AIRFRAME_COLORSETS = {
    "Professional Blue": {"C-130": "#2563eb", "C-27": "#7c3aed", "Custom_Transport": "#8b5cf6"},
    "Ocean Blue": {"C-130": "#0ea5e9", "C-27": "#06b6d4", "Custom_Transport": "#0891b2"},
    "Forest Green": {"C-130": "#059669", "C-27": "#16a34a", "Custom_Transport": "#10b981"},
    "Sunset Orange": {"C-130": "#f97316", "C-27": "#a855f7", "Custom_Transport": "#f59e0b"},
    "Midnight Purple": {"C-130": "#8b5cf6", "C-27": "#ec4899", "Custom_Transport": "#7c3aed"},
    "High Contrast": {"C-130": "#0080ff", "C-27": "#ffff00", "Custom_Transport": "#ff00ff"},
    "Cyber Green": {"C-130": "#00ff41", "C-27": "#00e63a", "Custom_Transport": "#00ffff"},
    "Desert Sand": {"C-130": "#b45309", "C-27": "#d97706", "Custom_Transport": "#d97706"},
    "Arctic Blue": {"C-130": "#0ea5e9", "C-27": "#0891b2", "Custom_Transport": "#0ea5e9"},
    "Classic Gray": {"C-130": "#6b7280", "C-27": "#9ca3af", "Custom_Transport": "#6b7280"},
}
...
class ThemeConfig:
    ...
    ac_colors: Dict[str, str] = field(default_factory=lambda: {"C-130": "#2563eb", "C-27": "#7c3aed", "Custom_Transport": "#8b5cf6"})
    bar_A: str = "#2563eb"
    bar_B: str = "#7c3aed"
    bar_C: str = "#059669"
    bar_D: str = "#dc2626"
...
class SimConfig:
    ...
    viz_show_aircraft_trails: bool = True
    ...
    # Time and distance mechanics configuration
    spoke_distances: List[float] = field(default_factory=lambda: [500.0] * M)  # Distance from hub to each spoke in miles
    variable_spoke_count: bool = False  # Enable dynamic spoke count
    max_spokes: int = 10  # Maximum number of spokes
    ...
    def to_json(self) -> dict:
        return {
            ...
            "viz_show_aircraft_trails": self.viz_show_aircraft_trails,
            ...
            "spoke_distances": self.spoke_distances,
            "variable_spoke_count": self.variable_spoke_count,
            "max_spokes": self.max_spokes,
    ...
    def from_json(d: dict) -> "SimConfig":
        ...
        cfg.viz_show_aircraft_trails = bool(d.get("viz_show_aircraft_trails", cfg.viz_show_aircraft_trails))
        ...
        cfg.spoke_distances = d.get("spoke_distances", cfg.spoke_distances)
        cfg.variable_spoke_count = bool(d.get("variable_spoke_count", cfg.variable_spoke_count))
        cfg.max_spokes = int(d.get("max_spokes", cfg.max_spokes))
...
def validate_config(cfg: SimConfig) -> List[str]:
    ...
    # Time and distance mechanics validation
    if cfg.max_spokes < 1 or cfg.max_spokes > 20:
        issues.append("Max spokes must be between 1 and 20")
    
    if cfg.cost_per_flight_hour <= 0:
        issues.append("Cost per flight hour must be positive")
    
    if cfg.turnover_time_multiplier <= 0:
        issues.append("Turnover time multiplier must be positive")
    
    if cfg.speed_units not in ["mach", "knots"]:
...
def load_config() -> SimConfig:
    ...
    if not USER_MAIN_CONFIG_FILE.exists() and MAIN_CONFIG_FILE.exists():
        try:
            import shutil
            USER_MAIN_CONFIG_FILE.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(MAIN_CONFIG_FILE, USER_MAIN_CONFIG_FILE)
        except Exception:
            pass
