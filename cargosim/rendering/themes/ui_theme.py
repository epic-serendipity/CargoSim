...
def apply_theme(root: tk.Misc, palette: Dict[str, str]):
    ...
    try:
        ...
        try:
            ...
            log_runtime_event("Failed to apply font manager", f"error={e}", level="WARNING")
...
def _apply_consistent_fonts(root, font_manager):
    ...
    def _apply_fonts_recursive(widget):
        try:
            ...
            # Apply appropriate fonts based on widget type using default tkinter fonts
            if 'Label' in widget_type:
                if hasattr(widget, 'cget') and 'style' in widget.configure():
                    try:
                        style = widget.cget('style')
                        if 'Header' in str(style):
                            widget.configure(font=DEFAULT_FONT_BOLD)
                        else:
            ...
            elif 'Entry' in widget_type or 'Combobox' in widget_type:
                widget.configure(font=DEFAULT_FONT)
